...
```bash
git clone https://github.com/kaushal-bakrania/fintrack-api.git
cd fintrack-api
```
...
The API is containerised and deployed to an Azure VM via Azure Container Registry.

```bash
# Build for Linux/AMD64
docker buildx build --platform linux/amd64 \
  -t fintrackregistry.azurecr.io/fintrack-api:latest --push .

# Pull and run on Azure VM
docker pull fintrackregistry.azurecr.io/fintrack-api:latest
docker run -d --name fintrack-api -p 80:8080 \
  -e ConnectionStrings__DefaultConnection="..." \
  -e Jwt__Key="..." \
  fintrackregistry.azurecr.io/fintrack-api:latest
```

GitHub Actions CI/CD pipeline triggers on every push to `main`.
